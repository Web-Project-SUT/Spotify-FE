  ...
  return (
    <>
      <div
        onClick={onClose}
        aria-hidden="true"
        className={`fixed inset-0 bg-black/60 z-40 transition-opacity md:hidden ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      />
      <aside
        className={`w-60 bg-surface flex-shrink-0 flex flex-col h-screen md:sticky top-0 p-4 pb-28 overflow-y-auto fixed inset-y-0 left-0 z-50 transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
        ...
        <div className="border-t border-border pt-4 mt-4">
          <Link
            href={user.role === 'listener' ? '/profile' : '#'}
            onClick={onClose}
